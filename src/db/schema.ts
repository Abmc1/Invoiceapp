...
// ---------------------------------------------------------------------------
// Recurring invoices (templates that generate real invoices on a schedule)
// ---------------------------------------------------------------------------
...
export const recurringInvoices = pgTable("recurring_invoices", {
  ...
  /** When the next invoice should be generated. Advanced by one frequency step every time the sweep fires — the field that makes re-running the sweep on the same day a no-op. */
  nextRunDate: timestamp("next_run_date", { withTimezone: true }).notNull(),
  /** Once nextRunDate would fall after this, the schedule auto-transitions to ENDED instead of generating another invoice. Optional — omit for an open-ended schedule. */
  ...
  index("recurring_invoices_next_run_date_idx").on(table.nextRunDate),
]);
...
export const recurringInvoiceItems = pgTable("recurring_invoice_items", {
  ...
  index("recurring_invoice_items_recurring_invoice_id_idx").on(table.recurringInvoiceId),
]);
...
export const invoices = pgTable("invoices", {
  ...
  /** Set when this invoice was auto-generated by a recurring schedule, rather than created by hand. Null'd out (not blocked) if the schedule is later deleted — the invoice itself must survive. */
  sourceRecurringInvoiceId: uuid("source_recurring_invoice_id").references(() => recurringInvoices.id, { onDelete: "set null" }),
...
export const invoicesRelations = relations(invoices, ({ one, many }) => ({
  ...
  sourceRecurringInvoice: one(recurringInvoices, {
    fields: [invoices.sourceRecurringInvoiceId],
...
export const recurringInvoicesRelations = relations(recurringInvoices, ({ one, many }) => ({
  ...
  createdBy: one(users, { fields: [recurringInvoices.createdByUserId], references: [users.id] }),
}));
...
export const recurringInvoiceItemsRelations = relations(recurringInvoiceItems, ({ one }) => ({
  ...
  service: one(services, { fields: [recurringInvoiceItems.serviceId], references: [services.id] }),
}));
