...
export function InvoiceForm({
  ...
  const [clientId, setClientId] = useState(defaultValues?.clientId ?? initialClientId ?? clients[0]?.id ?? "");
  const [issueDate, setIssueDate] = useState(() => defaultValues?.issueDate ?? new Date().toISOString().slice(0, 10));
  const [dueDate, setDueDate] = useState(
    () => defaultValues?.dueDate ?? new Date(Date.now() + defaultPaymentTermsDays * 86400000).toISOString().slice(0, 10)
  );
